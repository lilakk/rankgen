...
def scorer_rankgen(prefix, suffixes, prefix_vector=None):
...
def scorer_comet(prefix, suffixes):
...
def beam_search(contexts, k=10, beam_size=2, temperature=1.0, top_p=0.9, num_tokens=20, num_samples=10, max_length=115):
    final_outputs = []
    final_scores = []
    ...
    for ctx in contexts:
        if beam_size == 1 and num_samples == 1:
            ...
            _, prefix_vector, _ = scorer_rankgen(prefix=ctx, suffixes=[ctx])
        beams = [{
            "text": "",
            "eos": False
        ...
        while True:
            ...
            if len(all_outs) > 1:
                ...
                top_rankgen_scores = torch.Tensor([1.0])
                top_rankgen_scores.cuda()
                beams = all_outs
