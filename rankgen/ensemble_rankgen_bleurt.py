...
parser.add_argument('--output_file', default="rankgen_bleurt.jsonl", type=str)
...
def scorer_t5x(rankgen_encoder, prefix, suffixes, prefix_vector=None):
...
def beam_search(contexts, scorer=scorer_t5x, beam_size=2, temperature=1.0, top_p=0.9, num_tokens=20, num_samples=10, max_length=115):
        final_outputs = []
        final_scores = []
        ...
        for ctx in contexts:
            ...
            while True:
                ...
                if len(all_outs) > 1:
                    ...
                    bleurt_scores = bleurt_scorer.score(references=[prefix for _ in range(len(all_outs))], candidates=[x["text"] for x in all_outs])
                    bleurt_scores = [(s - torch.mean(torch.Tensor(bleurt_scores))) / torch.std(torch.Tensor(bleurt_scores)) for s in bleurt_scores]
                    ensemble_scores = [scores[i] + bleurt_scores[i] for i in range(len(scores))]
                    top_scores, top_indices = torch.topk(torch.Tensor(ensemble_scores), k=beam_size)
...
scorer_fn = partial(scorer_t5x, rankgen_encoder=rankgen_encoder)
