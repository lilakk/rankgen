...
parser.add_argument('--output_file', default="ensemble_expt/ensemble_rankgen_comet_product.jsonl", type=str)
...
def beam_search(contexts, beam_size=2, temperature=1.0, top_p=0.9, num_tokens=20, num_samples=10, max_length=115):
    final_outputs = []
    final_scores = []
    ...
    for ctx in contexts:
        if beam_size == 1 and num_samples == 1:
            ...
            _, prefix_vector, _ = scorer_rankgen(rankgen_encoder, prefix=ctx, suffixes=[ctx])
        beams = [{
            "text": "",
            "eos": False
        ...
        while True:
            ...
            if len(all_outs) > 1:
                ...
                rankgen_scores, _, _ = scorer_rankgen(rankgen_encoder, prefix=ctx, suffixes=[x["text"] for x in all_outs], prefix_vector=prefix_vector)
                rankgen_scores = normalize(rankgen_scores).to(device)
                comet_scores = scorer_comet(comet_model, prefix=ctx, suffixes=[x["text"] for x in all_outs])
                comet_scores = normalize(torch.Tensor(comet_scores)).to(device)
                ensemble_scores = rankgen_scores * comet_scores
                top_scores, top_indices = torch.topk(ensemble_scores, k=beam_size)
