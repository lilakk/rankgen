...
sim_tokenizer = AutoTokenizer.from_pretrained("princeton-nlp/unsup-simcse-roberta-base")
sim_model = AutoModel.from_pretrained("princeton-nlp/unsup-simcse-roberta-base")
sim_model.eval()
...
parser.add_argument('--output_file', default="ensemble_expt/simcse_full_rerank_unsup_roberta_base.jsonl", type=str)
...
def scorer_sim(sim_model, prefix, suffixes):
    prefix_tok = sim_tokenizer(prefix, padding=True, truncation=True, return_tensors="pt")
    suffixes_tok = sim_tokenizer(suffixes, padding=True, truncation=True, return_tensors="pt")
    with torch.no_grad():
        prefix_emb = sim_model(**prefix_tok, output_hidden_states=True, return_dict=True).pooler_output
        suffix_emb = sim_model(**suffixes_tok, output_hidden_states=True, return_dict=True).pooler_output
    similarities = torch.matmul(prefix_emb, suffix_emb.t()).squeeze(dim=0)
    return similarities
...
scorer_fn = partial(scorer_sim, sim_model=sim_model)
